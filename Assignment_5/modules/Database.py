...
class Database:
    ...
    def create_photo_card(self, front_photo_url, back_text):
        self.execute_query(self.insert_card(back_text), 'Insert card')
        self.cards += 1
        self.execute_query(self.insert_photo_card(self.cards, front_photo_url, back_text), 'Insert PhotoCard')

    def create_text_card(self, front_text, back_text):
        self.execute_query(self.insert_card(back_text), 'Insert card')
        self.cards += 1
        self.execute_query(self.insert_text_card(self.cards, front_text, back_text), 'Insert TextCard')

    def create_player(self, username, email, password, date_joined, league):
        self.execute_query(self.insert_user(username, email, password, date_joined), 'Insert User')
        self.users += 1
        self.execute_query(self.insert_player(self.users, league), 'Insert Player')

    def create_creator(self, username, email, password, date_joined):
        self.execute_query(self.insert_user(username, email, password, date_joined), 'Insert User')
        self.users += 1
        self.execute_query(self.insert_creator(self.users), 'Insert Creator')

    def create_cardset(self):
        self.execute_query(self.insert_cardset(), 'Insert Cardset')

    def create_score_leaderboard(self, competition_id, users_scores_places):
        self.execute_query(self.insert_leaderboard(competition_id), 'Insert Leaderboard')
        self.leaderboards += 1
        for user_id, score, place in users_scores_places:
            self.execute_query(self.insert_score_leaderboards(self.leaderboards, user_id, score, place), 'Insert ScoreLeaderboard')

    def create_time_leaderboard(self, competition_id, users_times_places):
        self.execute_query(self.insert_leaderboard(competition_id), 'Insert Leaderboard')
        self.leaderboards += 1
        for user_id, time, place in users_times_places:
            self.execute_query(self.insert_time_leaderboards(self.leaderboards, user_id, time, place), 'Insert TimeLeaderboard')

    def create_time_competition(self, leaderboard_id, start_time):
        self.execute_query(self.insert_competiton(leaderboard_id, 'TIME'), 'Insert Competition')
        self.competitions += 1
        self.execute_query(self.insert_time_competitions(self.competitions, leaderboard_id, start_time), 'Insert TimeCompetition')

    def create_score_competition(self, leaderboard_id, start_time):
        self.execute_query(self.insert_competiton(leaderboard_id, 'SCORE'), 'Insert Competition')
        self.competitions += 1
        self.execute_query(self.insert_time_competitions(self.competitions, leaderboard_id, start_time), 'Insert ScoreCompetition')

    def select_cardsets(self):
        return self.execute_query("select * from CARDSETS", "Selecting cardsets")

    def select_creators(self):
        return self.execute_query("select * from CREATORS", "Selecting creators")

    def select_players(self):
        return self.execute_query("select * from PLAYERS", "Selecting players")

    def select_competitions(self):
        return self.execute_query("select * from COMPETITIONS", "Selecting competitions")